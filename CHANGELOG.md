...
- Linux is driven by commands now, and they are words rather than switches:
  "TeamTalkMediaStreamer start apple", not "TeamTalkMediaStreamer --start apple". Running "TeamTalkMediaStreamer"
...
- Your files are now organised into config/, lang/, state/, auth/, cache/ and
  ...
  its own, but re-run "TeamTalkMediaStreamer service install" to update it properly.
...
- Running bots on Linux:
  ...
  - A bot that cannot reach its server no longer restarts every two seconds
    ...
    its log. Re-run "TeamTalkMediaStreamer service install" once to get this.
  ...
  - Running "TeamTalkMediaStreamer" on its own used to start whichever bot sorted first,
...
- Linux: a bot running as a systemd service no longer crashes (and gets
  ...
  to run `TeamTalkMediaStreamer --auth`, and keeps running with Spotify disabled
...
- New `lk` command: queues your Spotify Liked Songs.
...
- Linux: after `TeamTalkMediaStreamer --update` succeeds, the bot offers to restart your
...
- Self-updater: checks GitHub for a newer release and installs it (Windows via a
  tray dialog, Linux via `TeamTalkMediaStreamer --update`). Downloads are minisign-signed and
